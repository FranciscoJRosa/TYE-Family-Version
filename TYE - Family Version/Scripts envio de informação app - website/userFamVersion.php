...
if($_SERVER['REQUEST_METHOD']=='POST'){
  ...
  $userNatur = $_POST['userNatur'];
  $codigo = $_POST['codigo'];
  $password = $_POST['password'];

	//creating a statement to insert to database
	$stmt = $conn->prepare("INSERT INTO utilizador (username, userNatur, codigo, password) VALUES (?, ?, ?, ?)");

	//binding the parameter to statement
	$stmt->bind_param("ssss", $username, $userNatur, $codigo, $password);
